...
class GaussMethod:
	"""
	Utilities for Gauss Method [TODO: say what it does].
	...
	def __init__(
		...
		# init sun vectors
		# TODO: CHECK WHY HORIZONS API IS GIVING SLIGHTLY DIFF SUN VECTOR
		if sun_vectors.any() == None:
			# TODO: IMPLEMENT MORE ELEGANT INPUT FOR SUN_VECTORS
	...
	def _calc_gaussian_time_intervals(self):
		"""

		:return: array of tau values [tau1, tau0, tau3]
		"""
		tau1 = self.K_CONSTANT * (self.times[0] - self.times[1])
		tau0 = self.K_CONSTANT * (self.times[2] - self.times[0])
		tau3 = self.K_CONSTANT * (self.times[2] - self.times[1])
		return np.array([tau1, tau0, tau3])
	
	def _calc_initial_a_values(self, tau_values):
		"""
	...
	def _calc_rho_values(self, a_values, unit_range_vectors):
		"""
	...
	def _calc_general_r_vectors(self, unit_range_vectors, a_values):
		"""
	...
	def _calc_initial_r2_dot(self, r_vectors):
		"""

		:param r_vectors: np.ndarray of r_vectors [[r1], [r2], [r3]]
		"""
		# compute sub vels
		vel12 = (r_vectors[1] - r_vectors[0])/(self.times[1] - self.times[0])
		vel23 = (r_vectors[2] - r_vectors[1])/(self.times[2] - self.times[1])

		# compute initial r2dot
		initial_r2dot = (((self.times[2] - self.times[1])*vel12)+((self.times[1] - self.times[0])*vel23))/(self.times[2] - self.times[0])
		return initial_r2dot

	def _calc_fg_values(self, tau, r2, r2_dot):
		"""
	...
	def _calc_general_a_values(self, fg_values_1, fg_values_3):
		"""
	...
	def get_position_and_velocity(self):
		...
		tau_values = self._calc_gaussian_time_intervals() # tau is: [tau1, tau0, tau3]

		# calc initial values
		a_values = self._calc_initial_a_values(tau_values)

		print(a_values)
		r_vectors = self._calc_general_r_vectors(unit_range_vectors, a_values)
		r2 = r_vectors[1]
