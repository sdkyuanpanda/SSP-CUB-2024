...
Super cool orbital determination (OD) code and report stuff is available here. OD code is in the `od` folder and the main od file is titled `od_lib_yuan.py`. Experiments on asteroid 699 Hela (aka A910 LC) are available in a Jupyter Notebook—slides for the smaller orbital integration project (simulating the asteroid's orbit 50 Myr into the future) are also in the `od` folder. The final report is titled `ssp_team08_final_report-1.pdf` and is in the main directory. Below is a schematic of the OD process:
