...
def usr_orbit_element_errors(predicted_arr, true_arr, keys):
    ...
    for i in range(len(predicted_arr)):
        print(f"{i+1}: Element {keys[i]} || Rel Diff: {round(relative_difference(predicted_arr[i], true_arr[i]) * 100, 3)}% || Abs Diff: {round(absolute_difference(predicted_arr[i], true_arr[i]) * 100, 3)}")
...
def ra_to_deg(hours: int, mins: int, seconds: int, deg_status: bool = True) -> float:
    """
